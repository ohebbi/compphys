...
But what kind of error is this, and why does it decrease with increasing number of grid points?
The number of flops from the general algorithm is $5(N-1)$ from the forward substitution and $3N-2$ flops
from the backward substitution, making it a total of $8N-7$ flops. If $N$ is a very big number, we can simplify it to $8N$ total number flops for our general algorithm. Please check our code in our repository at GitHub for how we calculated the flops.
This results in CPU time in table (INGVILD). \\
We have not changed the backward substitution algorithm for the special case, but we have changed the forward substitution as described in the previous section. This results in a total number of $7N$ flops.
...
For computing $10^5 $x$ 10^5$ matrix with LU-decomposition (which goes)
