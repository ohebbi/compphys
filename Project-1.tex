...
First we wanted to write a script that would solve the equation $\mathbf{Av}=\mathbf{b}$ for general numbers, $a_i, b_i$ and $c_i$, so we assigned the vectors random values in a for loop. Later we wanted to specialize our code to the Poisson equation by defining $a_i=c_i=-1$ and $b_i=2$. These are the values we used for the rest of the project.
