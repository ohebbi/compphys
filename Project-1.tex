...
The theory behind the algorithms for forward and backward substitution are thoroughly described in the lecture notes (ref), and therefore we will not repeat it in this report.

First we wanted to write a script that would solve the equation $\mathbf{Av}=\mathbf{b}$ for general numbers, $a_i, b_i$ and $c_i$, so we assigned the vectors random values in a \textit{for loop}. Later we wanted to specialize our code to the Poisson equation by defining $a_i=c_i=-1$ and $b_i=2$ for $i=1, 2, ..., n$. These are the values we used for the rest of the project.
